 ...
 * Updated to support user authentication, membership cards, and activity tracking.
 */
public class Customer {
    ...
    private LocalDate lastActivityDate;     // Track last login/purchase for inactivity detection
    ...
    public Customer(String userID, String lastName, String firstName, String middleName, String password) {
        ...
        this.points = 0;
        this.isGuest = false;
        this.lastActivityDate = LocalDate.now();  // Set activity date to today on creation
    }
    ...
    /**
     * Returns the date of the customer's last activity (login or purchase).
     *
     * @return (LocalDate) The last activity date, or null if never set.
     */
    ...
    /**
     * Sets the customer's last activity date manually.
     * Useful for importing data or special cases.
     *
     * @param date (LocalDate) The date to set as last activity.
     */
     ...
     * Call this method when a customer logs in or completes a purchase.
     */
    ...
    /**
     * Checks if the account is inactive for more than 2 years.
     * An account is considered inactive if the last activity date is more than 730 days in the past.
     *
     * @return (boolean) True if inactive for 2+ years, false otherwise.
     */
    public boolean isInactiveForTwoYears() {
        if (lastActivityDate == null) {
            return true; // Consider null as very old/never active
        }
        long daysSinceLastActivity = ChronoUnit.DAYS.between(lastActivityDate, LocalDate.now());
        return daysSinceLastActivity > 730; // 2 years ≈ 730 days
    }

    /**
     * Gets the number of days since the customer's last activity.
     *
     * @return (long) Number of days since last activity, or Long.MAX_VALUE if never set.
     */
    ...
    /**
     * Checks if the account is approaching the 2-year inactivity threshold.
     * Useful for sending warning notifications (e.g., at 1.5 years).
     *
     * @param warningThresholdDays (long) The number of days to consider as "at risk".
     * @return (boolean) True if inactive for longer than the warning threshold.
     */
    public boolean isApproachingInactivityThreshold(long warningThresholdDays) {
        if (lastActivityDate == null) {
            return true;
        }
        long daysSinceLastActivity = ChronoUnit.DAYS.between(lastActivityDate, LocalDate.now());
        return daysSinceLastActivity > warningThresholdDays;
    }
